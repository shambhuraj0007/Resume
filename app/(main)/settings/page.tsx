...
export default function SettingsPage() {
  ...
  // Load settings from MongoDB when component mounts
  useEffect(() => {  // eslint-disable-line react-hooks/rules-of-hooks
    const loadSettings = async () => {
      if (!session?.user?.email) return;

      try {
        const response = await fetch(`/api/settings/${encodeURIComponent(session.user.email)}`);
        
        if (response.ok) {
          const data = await response.json();
          setSettings({
            displayName: data.settings.displayName,
            defaultTemplate: data.settings.defaultTemplate || 'modern'
          });
          
          // Also set to localStorage as backup
          localStorage.setItem('resumeitnow_name', data.settings.displayName);
          localStorage.setItem('resumeitnow_template', data.settings.defaultTemplate);
        } else {
  ...
  const handleAccountDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/user/${encodeURIComponent(session.user.email)}/delete`, {
        method: 'DELETE',
      });
